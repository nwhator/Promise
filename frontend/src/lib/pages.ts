...
/**
 * Load raw HTML for a stitch_promise_portfolio page.
 * Searches in:
 *   1. <project-root>/stitch_promise_portfolio
 *   2. <frontend>/stitch_promise_portfolio  (synced copy)
 */
export function loadPageHtml(page: PageEntry): string {
  const candidates = [
    path.join(process.cwd(), "..", "stitch_promise_portfolio", page.folder, "code.html"),
    path.join(process.cwd(), "stitch_promise_portfolio", page.folder, "code.html"),
  ];
  ...
  throw new Error(
    `Missing stitch HTML file for slug "${page.slug}". Tried:\n${candidates.join("\n")}`
  );
}

/**
 * Normalise links and inject route-resolution script into raw HTML.
 */
